...
### Opening worktrees in VS Code

The `Open in app...` action finds Visual Studio Code through the Windows `PATH` and standard per-user or system installation directories. It supports both the `code.exe`/`Code.exe` executable and the `code.cmd` launcher, including VS Code Insiders. A custom installation works when its `code.cmd` or `code.exe` is on `PATH`. If VS Code is not installed, Herdr reports that it could not find the launcher instead of silently opening the wrong application.
